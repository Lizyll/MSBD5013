...
class NeuralNetWork(object):
	...
	def testpla(self):
		...
		fixratio = 1/10000
		mthresh = 5
		t1 = tdata['winprob'].tolist()
		t2 = tdata['win_t5'].tolist()
		t5 = tdata['plaprob'].tolist()
		t6 = tdata['place_t5'].tolist()
		t3 = []
		t4 = []
		'''
		for i in range(len(tdata['winprob'])):
			if t5[i] * t5[i] != 0:
				t4.append(fixratio * random.randint(0, 1))
			else:
				t4.append(0)
		...
		for i in range(len(tdata['winprob'])):
			if t1[i] * t2[i] != 0:
				t3.append(fixratio * random.randint(0, 1))
			else:
				t3.append(0)
		...
		'''
		tdata['winstake'] = fixratio * (tdata['winprob'] * tdata['win_t5'] != 0)
		
		tdata['plastake'] = fixratio * (tdata['plaprob'] * tdata['place_t5'] != 0)
